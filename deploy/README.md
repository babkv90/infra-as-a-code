# infraFlowNodedeploymentPipeline deployment pipeline
...
aws iam create-role \
  --role-name infraflownodedeploymentpipeline-deploy-role \
...
aws iam put-role-policy \
  --role-name infraflownodedeploymentpipeline-deploy-role \
  --policy-name infraflownodedeploymentpipeline-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/infraflownodedeploymentpipeline-deploy-role`.
