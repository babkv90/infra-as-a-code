# noddeLambdachange deployment pipeline
...
aws iam create-role \
  --role-name noddelambdachange-deploy-role \
...
aws iam put-role-policy \
  --role-name noddelambdachange-deploy-role \
  --policy-name noddelambdachange-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/noddelambdachange-deploy-role`.
