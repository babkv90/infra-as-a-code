# ProductionLambda deployment pipeline
...
aws iam create-role \
  --role-name productionlambda-deploy-role \
...
aws iam put-role-policy \
  --role-name productionlambda-deploy-role \
  --policy-name productionlambda-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/productionlambda-deploy-role`.
