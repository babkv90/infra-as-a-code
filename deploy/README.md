# lmbdapipeline deployment pipeline
...
aws iam create-role \
  --role-name lmbdapipeline-deploy-role \
...
aws iam put-role-policy \
  --role-name lmbdapipeline-deploy-role \
  --policy-name lmbdapipeline-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/lmbdapipeline-deploy-role`.
