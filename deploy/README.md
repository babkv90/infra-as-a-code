# iaasInfraFlow deployment pipeline
...
aws iam create-role \
  --role-name iaasinfraflow-deploy-role \
...
aws iam put-role-policy \
  --role-name iaasinfraflow-deploy-role \
  --policy-name iaasinfraflow-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/iaasinfraflow-deploy-role`.
