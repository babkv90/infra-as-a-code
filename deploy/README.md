# ProductionLog deployment pipeline
...
aws iam create-role \
  --role-name productionlog-deploy-role \
...
aws iam put-role-policy \
  --role-name productionlog-deploy-role \
  --policy-name productionlog-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/productionlog-deploy-role`.
