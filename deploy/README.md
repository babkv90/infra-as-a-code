# LatestInfradeploymentpipeline deployment pipeline
...
aws iam create-role \
  --role-name latestinfradeploymentpipeline-deploy-role \
...
aws iam put-role-policy \
  --role-name latestinfradeploymentpipeline-deploy-role \
  --policy-name latestinfradeploymentpipeline-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/latestinfradeploymentpipeline-deploy-role`.
