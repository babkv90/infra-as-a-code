# NodeApppipeline deployment pipeline
...
aws iam create-role \
  --role-name nodeapppipeline-deploy-role \
...
aws iam put-role-policy \
  --role-name nodeapppipeline-deploy-role \
  --policy-name nodeapppipeline-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/nodeapppipeline-deploy-role`.
