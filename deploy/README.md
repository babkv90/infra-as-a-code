# productionReactPipeline deployment pipeline
...
aws iam create-role \
  --role-name productionreactpipeline-deploy-role \
...
aws iam put-role-policy \
  --role-name productionreactpipeline-deploy-role \
  --policy-name productionreactpipeline-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: set this as a repository secret or per-environment secret. Use the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/productionreactpipeline-deploy-role`.
