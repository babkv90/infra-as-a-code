# nodeAppDeployment deployment pipeline
...
aws iam create-role \
  --role-name nodeappdeployment-deploy-role \
...
aws iam put-role-policy \
  --role-name nodeappdeployment-deploy-role \
  --policy-name nodeappdeployment-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: set this as a repository secret or per-environment secret. Use the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/nodeappdeployment-deploy-role`.
