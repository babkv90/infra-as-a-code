...
aws iam create-role \
  --role-name awslambda-deploy-role \
...
aws iam put-role-policy \
  --role-name awslambda-deploy-role \
  --policy-name awslambda-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/awslambda-deploy-role`.
