# LambdaErrorResolver deployment pipeline
...
aws iam create-role \
  --role-name lambdaerrorresolver-deploy-role \
...
aws iam put-role-policy \
  --role-name lambdaerrorresolver-deploy-role \
  --policy-name lambdaerrorresolver-deploy-role-permissions \
...
- `AWS_DEPLOY_ROLE_ARN`: the ARN printed by step 2 above, e.g.
  `arn:aws:iam::<ACCOUNT_ID>:role/lambdaerrorresolver-deploy-role`.
