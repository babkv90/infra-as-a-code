...
function findRelatedVpc(node, nodes, edges, nodeById) {
...
function findRelatedSubnet(node, nodes, edges, nodeById) {
...
function connectedNode(node, edges, nodeById, serviceId) {
...
function isPublicSubnet(node) {
  return node.data?.groupKind === 'Public Subnet' || String(node.data?.config?.map_public_ip_on_launch ?? '').toLowerCase() === 'true' || String(node.data?.label ?? '').toLowerCase().includes('public');
}
