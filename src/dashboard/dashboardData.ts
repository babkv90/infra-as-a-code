...
export const commonDeploymentTemplates = [
  ...
  {
    id: 'testing-real-diagrams',
    name: 'Testing Real diagrams',
    compatibility: 'Serverless web apps with authenticated APIs, CRUD backends, static frontends with async processing',
    infrastructure: 'CloudFront, S3 (static site + assets), Cognito, API Gateway, Lambda (GET/POST/Processing/Authorizer), IAM role, DynamoDB, CloudWatch, SNS',
    deploymentPath: 'Deploy the static frontend to S3/CloudFront, publish Lambda handlers behind API Gateway with a Cognito authorizer, and wire CloudWatch alarms to SNS for alerting.',
  },
];
...
export const commonInfraTemplates: CommonInfraTemplate[] = commonDeploymentTemplates.map((template) => {
  const snapshots: Record<string, DiagramSnapshot> = {
    ...
    'testing-real-diagrams': templateSnapshot(
      [
        { id: 'tpl-trd-cloudfront', serviceId: 'cloudfront', label: 'CloudFront Distribution', x: 70, y: 500, config: { enabled: 'true', comment: 'Serverless web app CDN', default_root_object: 'index.html', price_class: 'PriceClass_100' } },
        { id: 'tpl-trd-s3-static', serviceId: 's3', label: 'S3 Static Website', x: 280, y: 500, config: { bucket_prefix: 'testing-real-static-', website_index_document: 'index.html', website_error_document: 'error.html', versioning: 'false' } },
        { id: 'tpl-trd-cognito', serviceId: 'cognito', label: 'Cognito User Pool', x: 480, y: 500, config: { name: 'testing-real-user-pool', mfa_configuration: 'OPTIONAL' } },
        { id: 'tpl-trd-apigw', serviceId: 'apigw', label: 'API Gateway', x: 80, y: 220, config: { name: 'testing-real-diagrams-api', protocol_type: 'HTTP' } },
        { id: 'tpl-trd-get', serviceId: 'lambda', label: 'GET Handler', x: 320, y: 110, config: { runtime: 'nodejs20.x', handler: 'index.handler', memory_size: 256, timeout: 15, filename: 'get-handler.zip', inline_filename: 'index.js', inline_source: 'exports.handler = async () => ({ statusCode: 200, body: JSON.stringify({ ok: true }) });' } },
        { id: 'tpl-trd-post', serviceId: 'lambda', label: 'POST Handler', x: 320, y: 220, config: { runtime: 'nodejs20.x', handler: 'index.handler', memory_size: 256, timeout: 15, filename: 'post-handler.zip', inline_filename: 'index.js', inline_source: 'exports.handler = async () => ({ statusCode: 201, body: JSON.stringify({ created: true }) });' } },
        { id: 'tpl-trd-authorizer', serviceId: 'lambda', label: 'Authorizer', x: 320, y: 330, config: { runtime: 'nodejs20.x', handler: 'index.handler', memory_size: 128, timeout: 10, filename: 'authorizer.zip', inline_filename: 'index.js', inline_source: 'exports.handler = async () => ({ isAuthorized: true });' } },
        { id: 'tpl-trd-processing', serviceId: 'lambda', label: 'Processing', x: 540, y: 220, config: { runtime: 'nodejs20.x', handler: 'index.handler', memory_size: 512, timeout: 60, filename: 'processing.zip', inline_filename: 'index.js', inline_source: 'exports.handler = async (event) => ({ processed: event.Records?.length ?? 0 });' } },
        { id: 'tpl-trd-role', serviceId: 'iam', label: 'Lambda Execution Role', x: 1000, y: 60, config: { name: 'testing-real-lambda-execution-role', assume_role_policy: 'data.aws_iam_policy_document.lambda_assume_role.json' } },
        { id: 'tpl-trd-dynamodb', serviceId: 'dynamodb', label: 'DynamoDB Tables', x: 770, y: 90, config: { billing_mode: 'PAY_PER_REQUEST', hash_key: 'id', hash_key_type: 'S' } },
        { id: 'tpl-trd-s3-assets', serviceId: 's3', label: 'S3 Assets Bucket', x: 770, y: 230, config: { bucket_prefix: 'testing-real-assets-', versioning: 'true' } },
        { id: 'tpl-trd-cloudwatch', serviceId: 'cloudwatch', label: 'CloudWatch', x: 770, y: 480, config: { comparison_operator: 'GreaterThanThreshold', evaluation_periods: 1, metric_name: 'Errors', namespace: 'AWS/Lambda', period: 300, statistic: 'Sum', threshold: 1 } },
        { id: 'tpl-trd-sns', serviceId: 'sns', label: 'SNS Notifications', x: 970, y: 480, config: { name: 'testing-real-diagrams-alerts', display_name: 'App Alerts' } },
      ],
      [
        { source: 'tpl-trd-cloudfront', target: 'tpl-trd-s3-static', label: 'origin', protocol: 'HTTPS' },
        { source: 'tpl-trd-cloudfront', target: 'tpl-trd-apigw', label: 'API calls', protocol: 'HTTPS' },
        { source: 'tpl-trd-cognito', target: 'tpl-trd-authorizer', label: 'validates token', connectionType: 'security', protocol: 'JWT' },
        { source: 'tpl-trd-apigw', target: 'tpl-trd-authorizer', label: 'authorizes request', connectionType: 'security', protocol: 'HTTP' },
        { source: 'tpl-trd-apigw', target: 'tpl-trd-get', label: 'routes GET', connectionType: 'event', protocol: 'HTTP' },
        { source: 'tpl-trd-apigw', target: 'tpl-trd-post', label: 'routes POST', connectionType: 'event', protocol: 'HTTP' },
        { source: 'tpl-trd-post', target: 'tpl-trd-processing', label: 'invokes', connectionType: 'event', protocol: 'Lambda' },
        { source: 'tpl-trd-get', target: 'tpl-trd-dynamodb', label: 'read', protocol: 'DynamoDB' },
        { source: 'tpl-trd-post', target: 'tpl-trd-dynamodb', label: 'write', protocol: 'DynamoDB' },
        { source: 'tpl-trd-processing', target: 'tpl-trd-dynamodb', label: 'updates', protocol: 'DynamoDB' },
        { source: 'tpl-trd-processing', target: 'tpl-trd-s3-assets', label: 'writes assets', protocol: 'S3' },
        { source: 'tpl-trd-role', target: 'tpl-trd-get', label: 'assumes', connectionType: 'security', protocol: 'IAM' },
        { source: 'tpl-trd-role', target: 'tpl-trd-post', label: 'assumes', connectionType: 'security', protocol: 'IAM' },
        { source: 'tpl-trd-role', target: 'tpl-trd-processing', label: 'assumes', connectionType: 'security', protocol: 'IAM' },
        { source: 'tpl-trd-role', target: 'tpl-trd-authorizer', label: 'assumes', connectionType: 'security', protocol: 'IAM' },
        { source: 'tpl-trd-get', target: 'tpl-trd-cloudwatch', label: 'logs/metrics', connectionType: 'monitoring', protocol: 'CloudWatch' },
        { source: 'tpl-trd-post', target: 'tpl-trd-cloudwatch', label: 'logs/metrics', connectionType: 'monitoring', protocol: 'CloudWatch' },
        { source: 'tpl-trd-processing', target: 'tpl-trd-cloudwatch', label: 'logs/metrics', connectionType: 'monitoring', protocol: 'CloudWatch' },
        { source: 'tpl-trd-cloudwatch', target: 'tpl-trd-sns', label: 'alerts', connectionType: 'monitoring', protocol: 'SNS' },
      ],
      [
        { id: 'tpl-trd-api-layer', kind: 'Module', label: 'API Layer', x: 40, y: 40, width: 660, height: 380 },
        { id: 'tpl-trd-lambda-functions', kind: 'Module', label: 'Lambda Functions', x: 270, y: 80, width: 400, height: 320 },
        { id: 'tpl-trd-data-layer', kind: 'Module', label: 'Data Layer', x: 740, y: 40, width: 240, height: 320 },
        { id: 'tpl-trd-monitoring', kind: 'Module', label: 'Monitoring & Notifications', x: 740, y: 440, width: 340, height: 160 },
        { id: 'tpl-trd-frontend', kind: 'Module', label: 'Frontend / Presentation Layer', x: 40, y: 460, width: 360, height: 160 },
        { id: 'tpl-trd-auth-layer', kind: 'Module', label: 'Authentication Layer', x: 440, y: 460, width: 220, height: 160 },
      ],
    ),
  };
