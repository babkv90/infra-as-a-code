import type { AwsEdge, AwsNode, InfrastructureRelationshipType } from '../types';
import { edgeResolvableFieldsForService } from './resourceRegistry';
...
export type RelationshipGraphIssue = {
  nodeId?: string;
  edgeId?: string;
...
export function buildRelationshipGraph(nodes: AwsNode[], edges: AwsEdge[]): InfrastructureRelationship[] {
  const nodeById = new Map(nodes.map((node) => [node.id, node]));
  const relationships: InfrastructureRelationship[] = [];
  ...
  for (const edge of edges) {
    ...
    const relationshipType = inferRelationshipType(source, target, edge);
    addRelationship(relationships, seen, {
      id: `edge-${edge.id}`,
...
export function validateRelationshipGraph(nodes: AwsNode[], edges: AwsEdge[]): RelationshipGraphIssue[] {
  const issues: RelationshipGraphIssue[] = [];
  ...
  for (const relationship of relationships) {
    const source = nodes.find((node) => node.id === relationship.sourceResourceId);
...
function inferRelationshipType(source: AwsNode, target: AwsNode, edge: AwsEdge): InfrastructureRelationshipType {
...
function addRelationship(relationships: InfrastructureRelationship[], seen: Set<string>, relationship: InfrastructureRelationship) {
...
function findCycles(relationships: InfrastructureRelationship[]): string[][] {
